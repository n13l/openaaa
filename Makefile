...
sub-make: FORCE
	$(Q)$(MAKE) --no-print-directory -C $(KBUILD_OUTPUT) KBUILD_SRC=$(CURDIR) \
...
# Objects we will link into libarch / subdirs we need to visit
endif # KBUILD_EXTMOD
...
objs-y += arch/$(SRCARCH) sys mem test tools $(BUILD_DIRS)
...
LDFLAGS_libarch += $(LDFLAGS_BUILD_ID)
...
#
export INSTALL_DTBS_PATH ?= $(INSTALL_PATH)/dtbs/$(PACKAGERELEASE)
...
export KBUILD_ALLDIRS := $(sort $(filter-out arch/%,$(package-dirs)) \
                         arch sys/$(PLATFORM) sys/unix mem test tools \
...
ifdef CONFIG_GDB_SCRIPTS
	$(Q)ln -fsn `cd $(srctree) && /bin/pwd`/scripts/gdb/libarch-gdb.py
endif

define filechk_package.release
...
# PACKAGERELEASE can change from a few different places, meaning version.h
...
define filechk_version.h
	(echo \#define PACKAGE_VERSION_CODE $(shell                         \
	expr $(VERSION) \* 65536 + 0$(PATCHLEVEL) \* 256 + 0$(SUBLEVEL)); \
	echo '#define PACKAGE_VERSION(a,b,c) (((a) << 16) + ((b) << 8) + (c))';)
endef
...
export INSTALL_HDR_PATH = $(objtree)/lib
...
libarchclean:
	$(Q)$(CONFIG_SHELL) $(srctree)/scripts/link-libarch.sh clean

clean: archclean libarchclean
...
%pkg: include/config/package.release FORCE
	$(Q)$(MAKE) $(build)=$(package-dir) $@
rpm: include/config/package.release FORCE
	$(Q)$(MAKE) $(build)=$(package-dir) $@
...
help:
	...
	@echo  '  dir/file.$(SO)  - Build module including final link'
...
quiet_cmd_depmod = DEPMOD  $(PACKAGERELEASE)
      cmd_depmod = $(CONFIG_SHELL) $(srctree)/scripts/depmod.sh $(DEPMOD) \
                   $(PACKAGERELEASE) "$(patsubst y,_,$(CONFIG_HAVE_UNDERSCORE_SYMBOL_PREFIX))"
