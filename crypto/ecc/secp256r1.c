...
static void
secp256r1_key_exchange(const u8 *public, const u8 *secret, u8 *shared)
{
	ecdh_shared_secret(public, secret, shared);
}

static void
secp256r1_key_pair(u8 *public_key, u8 *private_key)
{
}

static void
secp256r1_derive(const u8* seed, const u8 *public, u8 *shared)
{
}

struct ecc_group secp256r1_ecc_group = {
	.name = "secp256r1",
	.id = ECC_GROUP_SECP256R1,
	.pub_key_len = ECC_BYTES,
	.prv_key_len = ECC_BYTES,
	.key_pair = secp256r1_key_pair,
	.key_exchange = secp256r1_key_exchange,
	.size = 32,
...
{
	crypto_ecc_group_register(&secp256r1_ecc_group);
}
