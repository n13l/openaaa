...
static int 
ecc_make_key(uint8_t p_publicKey[ECC_BYTES+1], uint8_t p_privateKey[ECC_BYTES])
{
...
static int 
ecdh_shared_secret(const uint8_t p_publicKey[ECC_BYTES+1], const uint8_t p_privateKey[ECC_BYTES], uint8_t p_secret[ECC_BYTES])
{
    EccPoint l_public;
    uint64_t l_private[NUM_ECC_DIGITS];
...
static int 
ecdsa_verify(const uint8_t p_publicKey[ECC_BYTES+1], const uint8_t p_hash[ECC_BYTES], const uint8_t p_signature[ECC_BYTES*2])
{
...
static void
secp521r1_key_exchange(const u8 *public, const u8 *secret, u8 *shared)
{
	ecdh_shared_secret(public, secret, shared);
}

static void
secp521r1_key_pair(u8 *public_key, u8 *private_key)
{
}

static void
secp521r1_derive(const u8* seed, const u8 *public, u8 *shared)
{
}

struct ecc_group secp521r1_ecc_group = {
	.name = "secp521r1",
	.id = ECC_GROUP_SECP521R1,
	.pub_key_len = ECC_BYTES,
	.prv_key_len = ECC_BYTES,
	.key_pair = secp521r1_key_pair,
	.key_exchange = secp521r1_key_exchange,
	.size = 65,
	.derive = secp521r1_derive,
};

static void __init__ secp521r1_init(void)
{
	crypto_ecc_group_register(&secp521r1_ecc_group);
}
