...
static void
secp128r1_key_exchange(const u8 *public, const u8 *secret, u8 *shared)
{
	ecdh_shared_secret(public, secret, shared);
}

static void
secp128r1_key_pair(u8 *public_key, u8 *private_key)
{
}

static void
secp128r1_derive(const u8* seed, const u8 *public, u8 *shared)
{
}

struct ecc_group secp128r1_ecc_group = {
	.name = "secp128r1",
	.id = ECC_GROUP_SECP128R1,
	.pub_key_len = ECC_BYTES,
	.prv_key_len = ECC_BYTES,
	.key_pair = secp128r1_key_pair,
	.key_exchange = secp128r1_key_exchange,
	.size = 16,
	.derive = secp128r1_derive,
};

static void __init__ secp128r1_init(void)
{
	crypto_ecc_group_register(&secp128r1_ecc_group);
}
