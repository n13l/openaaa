#include <sys/compiler.h>
#include <sys/cpu.h>

#include <stdio.h>                                                              
#include <stdlib.h>                                                             
#include <string.h>

#include <crypto/sha256.h>

#define DBL_INT_ADD(a,b,c) if (a > 0xffffffff - (c)) ++b; a += c;
#define ROTLEFT(a,b) (((a) << (b)) | ((a) >> (32 - (b))))
#define ROTRIGHT(a,b) (((a) >> (b)) | ((a) << (32 - (b))))

#define CH(x,y,z) (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x,y,z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
#define EP0(x) (ROTRIGHT(x,2) ^ ROTRIGHT(x,13) ^ ROTRIGHT(x,22))
#define EP1(x) (ROTRIGHT(x,6) ^ ROTRIGHT(x,11) ^ ROTRIGHT(x,25))
#define SIG0(x) (ROTRIGHT(x,7) ^ ROTRIGHT(x,18) ^ ((x) >> 3))
#define SIG1(x) (ROTRIGHT(x,17) ^ ROTRIGHT(x,19) ^ ((x) >> 10))

static u32 sha256_k[SHA256_BLOCK_SIZE] = {
	0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,
	0x923f82a4,0xab1c5ed5,
	0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,
	0x9bdc06a7,0xc19bf174,
	0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,
	0x5cb0a9dc,0x76f988da,
	0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,
	0x06ca6351,0x14292967,
	0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,
	0x81c2c92e,0x92722c85,
	0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,
	0xf40e3585,0x106aa070,
	0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,
	0x5b9cca4f,0x682e6ff3,
	0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,
	0xbef9a3f7,0xc67178f2
};

void
sha256_transform(struct sha256 *sha256, byte data[])
{
	unsigned int a,b,c,d,e,f,g,h,i,j,t1,t2,m[SHA256_BLOCK_SIZE];

	for (i = 0,j = 0; i < 16; ++i, j += 4)
		m[i] = (data[j] << 24) | (data[j + 1] << 16) | (data[j + 2] <<
		       8) | (data[j + 3]);
	for ( ; i < SHA256_BLOCK_SIZE; ++i)
		m[i] = SIG1(m[i - 2]) + m[i - 7] + SIG0(m[i - 15]) + m[i - 16];

	a = sha256->state[0];
	b = sha256->state[1];
	c = sha256->state[2];
	d = sha256->state[3];
	e = sha256->state[4];
	f = sha256->state[5];
	g = sha256->state[6];
	h = sha256->state[7];
   
	for (i = 0; i < SHA256_BLOCK_SIZE; ++i) {
		t1 = h + EP1(e) + CH(e,f,g) + sha256_k[i] + m[i];
		t2 = EP0(a) + MAJ(a,b,c);
		h = g;
		g = f;
		f = e;
		e = d + t1;
		d = c;
		c = b;
		b = a;
		a = t1 + t2;
	}

	sha256->state[0] += a;
	sha256->state[1] += b;
	sha256->state[2] += c;
	sha256->state[3] += d;
	sha256->state[4] += e;
	sha256->state[5] += f;
	sha256->state[6] += g;
	sha256->state[7] += h;
}  

void sha256_init(struct sha256 *sha256)
{  
	sha256->len = 0; 
	sha256->bitlen[0] = 0; 
	sha256->bitlen[1] = 0; 
	sha256->state[0] = 0x6a09e667;
	sha256->state[1] = 0xbb67ae85;
	sha256->state[2] = 0x3c6ef372;
	sha256->state[3] = 0xa54ff53a;
	sha256->state[4] = 0x510e527f;
	sha256->state[5] = 0x9b05688c;
	sha256->state[6] = 0x1f83d9ab;
	sha256->state[7] = 0x5be0cd19;
}

void
sha256_update(struct sha256 *sha256, byte data[], unsigned int len)
{  
	for (unsigned int i = 0; i < len; ++i) { 
		sha256->data[sha256->len] = data[i]; 
		sha256->len++; 
		if (sha256->len == SHA256_BLOCK_SIZE) { 
			sha256_transform(sha256,sha256->data);
			DBL_INT_ADD(sha256->bitlen[0],sha256->bitlen[1],512); 
			sha256->len = 0; 
		}  
	}  
}  

void
sha256_final(struct sha256 *sha256, byte hash[])
{  
	unsigned int i = sha256->len; 
   
	if (sha256->len < 56) { 
		sha256->data[i++] = 0x80; 
		while (i < 56) 
			sha256->data[i++] = 0x00; 
	} else { 
		sha256->data[i++] = 0x80; 
		while (i < SHA256_BLOCK_SIZE) 
			sha256->data[i++] = 0x00; 
		sha256_transform(sha256,sha256->data);
		memset(sha256->data,0,56); 
	}  
   
	DBL_INT_ADD(sha256->bitlen[0],sha256->bitlen[1],sha256->len * 8);
	sha256->data[63] = sha256->bitlen[0]; 
	sha256->data[62] = sha256->bitlen[0] >> 8; 
	sha256->data[61] = sha256->bitlen[0] >> 16; 
	sha256->data[60] = sha256->bitlen[0] >> 24; 
	sha256->data[59] = sha256->bitlen[1]; 
	sha256->data[58] = sha256->bitlen[1] >> 8; 
	sha256->data[57] = sha256->bitlen[1] >> 16;  
	sha256->data[56] = sha256->bitlen[1] >> 24; 
	sha256_transform(sha256,sha256->data);
   
	/* SHA uses CPU_BIG_ENDIAN */
#ifdef CPU_LITTLE_ENDIAN
	for (i=0; i < 4; ++i) { 
		hash[i]      = (sha256->state[0] >> (24 - i * 8)) & 0x000000ff; 
		hash[i + 4]  = (sha256->state[1] >> (24 - i * 8)) & 0x000000ff; 
		hash[i + 8]  = (sha256->state[2] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 12] = (sha256->state[3] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 16] = (sha256->state[4] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 20] = (sha256->state[5] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 24] = (sha256->state[6] >> (24 - i * 8)) & 0x000000ff;
		hash[i + 28] = (sha256->state[7] >> (24 - i * 8)) & 0x000000ff;
	}
#endif/*CPU_LITTLE_ENDIAN*/
}  
