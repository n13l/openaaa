...
static void 
crc32_update_by1(struct crc32 *crc32, const byte *buf, unsigned int len)
{
	u32 crc = crc32->state;
	while (len--)
		crc = table_il8_o32[(crc ^ *buf++) & 0x000000FF] ^ (crc >> 8);
	crc32->state = crc;
}

static void
crc32_update_by4(struct crc32 *crc32, const byte *buf, unsigned int len)
{
	unsigned int init_bytes, words;
	u32 crc = crc32->state;
	...
	if (init_bytes) {
		...
		len -= init_bytes;
		while (init_bytes--)
			crc = table_il8_o32[(crc ^ *buf++) & 0x000000FF] ^
			     (crc >> 8);
	}
	...
	while (words--) {
		crc ^= *buf32++;
		term1 = table_il8_o56[crc & 0x000000FF] ^ 
		        table_il8_o48[(crc >> 8) & 0x000000FF];
		term2 = crc >> 16;
		crc = term1 ^
		      table_il8_o40[term2 & 0x000000FF] ^
		      table_il8_o32[(term2 >> 8) & 0x000000FF];
	}
...
static void
crc32_update_by8(struct crc32 *crc32, const byte *buf, unsigned int len)
{
	unsigned int init_bytes, quads;
	u32 crc = crc32->state;
	u32 term1, term2, *buf32;
	...
	while (quads--) {
		crc ^= *buf32++;
		term1 = table_il8_o88[crc & 0x000000FF] ^
		table_il8_o80[(crc >> 8) & 0x000000FF];
		term2 = crc >> 16;
		crc = term1 ^
		table_il8_o72[term2 & 0x000000FF] ^
		table_il8_o64[(term2 >> 8) & 0x000000FF];
		term1 = table_il8_o56[*buf32 & 0x000000FF] ^
		table_il8_o48[(*buf32 >> 8) & 0x000000FF];
...
u32
crc32_generate(const byte *buf, unsigned int len)
{
	u32 res, c32 = ~0L;
	byte b0,b1,b2,b3;
...
crc32_init(struct crc32 *crc32, unsigned int mode)
{
	...
	case CRC_MODE_DEFAULT:
		crc32->update = crc32_update_by4;
		break;
	case CRC_MODE_SMALL:
		crc32->update = crc32_update_by1;
		break;
	case CRC_MODE_BIG:
		crc32->update = crc32_update_by8;
		break;
	default:
		assert(0);
	}
...
{
	struct crc32 crc32;
	crc32_init(&crc32, CRC_MODE_DEFAULT);
	crc32_update(&crc32, buf, len);

	return crc32_final(&crc32);
}
