#ifndef __X86_ELF_LIB_H__
#define __X86_ELF_LIB_H__

#include <sys/compiler.h>
#include <sys/cpu.h>

const char *elf_abi_names(u32 id);
const char *elf_segment_types(u32 id);

#define ELF_HDR_SIZE                    16

struct elf32_ehdr {
	byte       ident[ELF_HDR_SIZE]; /* Magic number and other info */
...
struct elf32_sym {
	...
	byte    info;		/* Symbol type and binding */
	byte    other;		/* Symbol visibility */
...
struct elf32_syminfo {
	u16 boundto;		/* Direct bindings, symbol bound to */
	u16 flags;			/* Per symbol flags */
};
...
#define ELF32_ST_BIND(val)		(((byte) (val)) >> 4)
...
struct elf32_rel {
	u32	offset;		/* Address */
	u32	info;			/* Relocation type and symbol index */
};
...
struct elf32_verdef {
	u16	version;		/* Version revision */
	u16	flags;		/* Version information */
	u16	ndx;			/* Version Index */
	u16	cnt;			/* Number of associated aux entries */
	u32	hash;		/* Version name hash value */
	u32	aux;			/* Offset in bytes to verdaux array */
	u32	next;		/* Offset in bytes to next verdef entry */
};
...
struct elf32_verdaux {
	u32	name;		/* Version or dependency names */
	u32	next;		/* Offset in bytes to next verdaux entry */
};
...
struct elf32_verneed {
	u16	version;		/* Version of structure */
	u16	cnt;			/* Number of associated aux entries */
	u32	file;		/* Offset of filename for this dependency */
	u32	aux;			/* Offset in bytes to vernaux array */
	u32	next;		/* Offset in bytes to next verneed entry */
};
...
struct elf32_vernaux {
	u32	hash;		/* Hash value of dependency name */
	u16	flags;		/* Dependency specific information */
	u16	other;		/* Unused */
	u32	name;		/* Dependency name string offset */
	u32	next;		/* Offset in bytes to next vernaux entry */
};
...
struct elf32_nhdr {
	u32 namesz;			/* Length of the note's name.  */
	u32 descsz;			/* Length of the note's descriptor.  */
	u32 type;			/* Type of the note.  */
};
...
struct elf32_move {
	u64 value;		/* Symbol value.  */
	u32 info;		/* Size and index.  */
	u32 poffset;		/* Symbol offset.  */
	u16 repeat;		/* Repeat count.  */
	u16 stride;		/* Stride info.  */
};

struct elf64_move {
	u64 value;		/* Symbol value.  */
	u64 info;		/* Size and index.  */
	u64 poffset;	/* Symbol offset.  */
	u16 repeat;		/* Repeat count.  */
	u16 stride;		/* Stride info.  */
};
...
#define ELF32_M_SIZE(info)	((byte) (info))
#define ELF32_M_INFO(sym, size)	(((sym) << 8) + (byte) (size))
...
union elf32_gptab {
	struct {
		u32 current_g_value;	/* -G value used for compilation.  */
		u32 unused;		/* Not used.  */
	} gt_header;			/* First entry in section.  */
	struct {
		u32 g_value;		/* If this value were used for -G.  */
		u32 bytes;		/* This many bytes would be used.  */
...
struct elf32_reginfo {
	u32 gprmask;		/* General registers used.  */
	u32 cprmask[4];		/* Coprocessor registers used.  */
	s32 gp_value;		/* $gp register value.  */
};

/* Entries found in sections of type SHT_MIPS_OPTIONS.  */

struct elf_options {
	byte kind;		/* Determines interpretation of the
				   variable part of descriptor.  */
	byte size;		/* Size of descriptor, including header.  */
	u16 section;	/* Section header index of section affected,
